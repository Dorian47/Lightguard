...
#define WPA_PTK_TCP_PORT 9899
#define WPA_PTK_TCP_MAX_LEN (2 * (32 + WPA_TK_MAX_LEN))

static void wpa_auth_ptk_tcp_handler(int sock, void *eloop_ctx,
				   void *sock_ctx);
static void wpa_auth_ptk_tcp_init(struct wpa_authenticator *wpa_auth);
static void wpa_auth_ptk_tcp_deinit(struct wpa_authenticator *wpa_auth);
...
static void wpa_rekey_ptk(void *eloop_ctx, void *timeout_ctx)
{
	 ...
	 */
	if (sm->pairwise != WPA_CIPHER_NONE &&
	    wpa_cipher_valid_pairwise(sm->pairwise)) {
		...
		if (klen > 0 && (unsigned int) klen == sm->PTK.tk_len &&
		    wpa_auth_set_key(sm->wpa_auth, 0, alg, sm->addr, 0,
				     sm->PTK.tk, klen,
...
struct wpa_auth_ptk_update_ctx {
	const char *hex;
	size_t hexlen;
	int success;
	int fail;
...
static int wpa_auth_install_custom_ptk_cb(struct wpa_state_machine *sm,
						  void *ctx)
{
	struct wpa_auth_ptk_update_ctx *u = ctx;
	int klen;
	int expected_len;
	...
	if (klen <= 0 || klen > WPA_TK_MAX_LEN) {
		u->fail++;
		return 0;
	}

	expected_len = 32 + klen;
	if (u->hexlen != (size_t) (2 * expected_len)) {
		wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_WARNING,
				"PTK TCP: invalid length for current cipher");
		u->fail++;
		return 0;
	}

	if (hexstr2bin(u->hex, keybuf, expected_len) < 0) {
		wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_WARNING,
	...
	os_memcpy(ptk.kck, keybuf, 16);
	os_memcpy(ptk.kek, keybuf + 16, 16);
	os_memcpy(ptk.tk, keybuf + 32, klen);
	ptk.kck_len = 16;
	ptk.kek_len = 16;
	ptk.tk_len = klen;
				   ...
				   void *sock_ctx)
{
	...
	char buf[WPA_PTK_TCP_MAX_LEN + 32];
	int client;
	int len;
	char *hex;
	struct wpa_auth_ptk_update_ctx ctx;
	...
	if (*hex == '\0') {
		wpa_auth_logger(wpa_auth, NULL, LOGGER_WARNING,
				"PTK TCP: empty payload");
		return;
	}

	ctx.hex = hex;
	ctx.hexlen = os_strlen(hex);
	ctx.success = 0;
	ctx.fail = 0;

	wpa_auth_for_each_sta(wpa_auth, wpa_auth_install_custom_ptk_cb, &ctx);
	wpa_auth_vlogger(wpa_auth, NULL, LOGGER_INFO,
			 "PTK TCP: update done (ok=%d fail=%d)",
			 ctx.success, ctx.fail);
}
...
static void wpa_auth_ptk_tcp_init(struct wpa_authenticator *wpa_auth)
{
	...
	addr.sin_port = htons(WPA_PTK_TCP_PORT);
	...
	if (eloop_register_read_sock(s, wpa_auth_ptk_tcp_handler,
				    wpa_auth, NULL) < 0) {
		wpa_auth_logger(wpa_auth, NULL, LOGGER_WARNING,
	...
	wpa_auth->ptk_tcp_sock = s;
	wpa_auth_vlogger(wpa_auth, NULL, LOGGER_INFO,
			"PTK TCP: listening on 127.0.0.1:%d", WPA_PTK_TCP_PORT);
}


static void wpa_auth_ptk_tcp_deinit(struct wpa_authenticator *wpa_auth)
{
	if (wpa_auth->ptk_tcp_sock >= 0) {
		eloop_unregister_read_sock(wpa_auth->ptk_tcp_sock);
		close(wpa_auth->ptk_tcp_sock);
		wpa_auth->ptk_tcp_sock = -1;
	}
}
				    ...
				    void *cb_ctx)
{
	...
	wpa_auth_ptk_tcp_init(wpa_auth);
...
void wpa_deinit(struct wpa_authenticator *wpa_auth)
{
	...
	wpa_auth_ptk_tcp_deinit(wpa_auth);
			    ...
			    struct wpa_state_machine *sm)
{
	...
	/*
	 * 跳过四次握手：在 STA 关联后，直接使用自定义 PTK 并安装到驱动
	 */
	extern const struct wpa_ptk g_custom_test_ptk;
	...
	if (sm->pairwise != WPA_CIPHER_NONE &&
		...
		if (klen > 0 && (unsigned int) klen == sm->PTK.tk_len) {
			if (wpa_auth_set_key(sm->wpa_auth, 0, alg, sm->addr, 0,
				...
				/* 安装自定义 GTK 到驱动 */
				if (sm->group && sm->group->GTK_len > 0) {
					enum wpa_alg gtk_alg = wpa_cipher_to_alg(wpa_auth->conf.wpa_group);
					int gtk_len = sm->group->GTK_len;
					
					/* 使用自定义GTK覆盖group中的GTK */
					if (gtk_len <= (int) sizeof(g_custom_test_gtk)) {
						os_memcpy(sm->group->GTK[sm->group->GN - 1],
							  g_custom_test_gtk, gtk_len);
						
						/* 安装GTK到驱动（使用广播地址和group key index） */
						if (wpa_auth_set_key(sm->wpa_auth, sm->group->vlan_id,
								      gtk_alg, broadcast_ether_addr,
								      sm->group->GN,
								      sm->group->GTK[sm->group->GN - 1],
								      gtk_len,
				...
				wpa_auth_set_eapol(sm->wpa_auth, sm->addr,
						   WPA_EAPOL_keyAvailable, false);
				wpa_auth_set_eapol(sm->wpa_auth, sm->addr,
						   WPA_EAPOL_keyDone, true);
				...
				wpa_auth_logger(wpa_auth, sm->addr, LOGGER_INFO,
						"TEST: Custom PTK and GTK installed, 4-way handshake skipped, authorized=1");
				return 0;
			} else {
				wpa_auth_logger(wpa_auth, sm->addr, LOGGER_WARNING,
						"TEST: Failed to install custom PTK");
			}
		}
	}
		 ...
		 u8 *data, size_t data_len)
{
	...
	/*
	 * 跳过四次握手：如果已经安装了自定义 PTK，忽略所有 EAPOL-Key 报文
	 */
	if (sm->PTK_valid && sm->pairwise_set) {
		wpa_auth_logger(wpa_auth, sm->addr, LOGGER_DEBUG,
...
int fils_set_tk(struct wpa_state_machine *sm)
{
	...
	/*
	 * 覆盖：在真正把 TK 装到驱动之前，用自定义 PTK
	 * 覆盖掉握手过程中计算出的 sm->PTK。
	 * 这里使用与 STA 端相同的 g_custom_test_ptk。
	 */
	extern const struct wpa_ptk g_custom_test_ptk;
	os_memcpy(&sm->PTK, &g_custom_test_ptk, sizeof(sm->PTK));
