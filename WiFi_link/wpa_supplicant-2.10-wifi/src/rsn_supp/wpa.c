...
#define WPA_SUPP_PTK_TCP_PORT 9900
#define WPA_SUPP_PTK_TCP_MAX_LEN (2 * (32 + WPA_TK_MAX_LEN))

static void wpa_sm_ptk_tcp_handler(int sock, void *eloop_ctx,
				  void *sock_ctx);
static void wpa_sm_ptk_tcp_init(struct wpa_sm *sm);
static void wpa_sm_ptk_tcp_deinit(struct wpa_sm *sm);
...
static void wpa_sm_rekey_ptk(void *eloop_ctx, void *timeout_ctx)
{
	...
	/*
	 * TEST: fast PTK switch without 4-way handshake. Reinstall custom PTK
	 * directly to keep traffic running with minimal downtime.
	 */
	if (sm->pairwise_cipher != WPA_CIPHER_NONE &&
	    wpa_cipher_valid_pairwise(sm->pairwise_cipher)) {
		extern const struct wpa_ptk g_custom_test_ptk;
		enum wpa_alg alg = wpa_cipher_to_alg(sm->pairwise_cipher);
		int keylen = wpa_cipher_key_len(sm->pairwise_cipher);
		int rsclen = wpa_cipher_rsc_len(sm->pairwise_cipher);

		os_memcpy(&sm->ptk, &g_custom_test_ptk, sizeof(sm->ptk));
		sm->ptk_set = 1;

		if (keylen > 0 && (unsigned int) keylen == sm->ptk.tk_len &&
		    wpa_sm_set_key(sm, alg, sm->bssid, sm->keyidx_active, 1,
				 null_rsc, rsclen, sm->ptk.tk, keylen,
				 KEY_FLAG_PAIRWISE_RX_TX) == 0) {
			wpa_dbg(sm->ctx->msg_ctx, MSG_INFO,
				"TEST: Fast PTK rekey install success");
			...
			if (sm->wpa_ptk_rekey) {
				...
				eloop_register_timeout(sm->wpa_ptk_rekey, 0,
							   wpa_sm_rekey_ptk,
							   sm, NULL);
			}
			return;
		...
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
			"TEST: Fast PTK rekey install failed; fallback to 4-way");
	}

	/* Fallback to normal PTK rekeying */
				  ...
				  void *sock_ctx)
{
	...
	char buf[WPA_SUPP_PTK_TCP_MAX_LEN + 32];
	int client;
	int len;
	char *hex;
	u8 keybuf[32 + WPA_TK_MAX_LEN];
	int keylen;
	int rsclen;
	int expected_len;
	enum wpa_alg alg;
	...
	if (sm->pairwise_cipher == WPA_CIPHER_NONE ||
	    !wpa_cipher_valid_pairwise(sm->pairwise_cipher)) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
			"PTK TCP: invalid pairwise cipher");
		return;
	}

	keylen = wpa_cipher_key_len(sm->pairwise_cipher);
	if (keylen <= 0 || keylen > WPA_TK_MAX_LEN) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
			"PTK TCP: invalid key length");
		return;
	}

	expected_len = 32 + keylen;
	if (os_strlen(hex) != (size_t) (2 * expected_len)) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
			"PTK TCP: invalid hex length");
		return;
	}

	if (hexstr2bin(hex, keybuf, expected_len) < 0) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
			"PTK TCP: hex parse failed");
		return;
	}

	os_memset(&sm->ptk, 0, sizeof(sm->ptk));
	os_memcpy(sm->ptk.kck, keybuf, 16);
	os_memcpy(sm->ptk.kek, keybuf + 16, 16);
	os_memcpy(sm->ptk.tk, keybuf + 32, keylen);
	sm->ptk.kck_len = 16;
	sm->ptk.kek_len = 16;
	sm->ptk.tk_len = keylen;
	sm->ptk_set = 1;
	...
	if (wpa_sm_set_key(sm, alg, sm->bssid, sm->keyidx_active, 1,
			 ...
			 KEY_FLAG_PAIRWISE_RX_TX) < 0) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
			"PTK TCP: install failed");
		return;
	}

	wpa_hexdump_key(MSG_INFO, "PTK TCP: KCK", sm->ptk.kck, sm->ptk.kck_len);
	wpa_hexdump_key(MSG_INFO, "PTK TCP: KEK", sm->ptk.kek, sm->ptk.kek_len);
	wpa_hexdump_key(MSG_INFO, "PTK TCP: TK", sm->ptk.tk, sm->ptk.tk_len);
	...
	if (sm->wpa_ptk_rekey) {
		eloop_cancel_timeout(wpa_sm_rekey_ptk, sm, NULL);
		eloop_register_timeout(sm->wpa_ptk_rekey, 0,
					 wpa_sm_rekey_ptk, sm, NULL);
	}

	wpa_dbg(sm->ctx->msg_ctx, MSG_INFO,
		"PTK TCP: custom PTK installed");
}
...
static void wpa_sm_ptk_tcp_init(struct wpa_sm *sm)
{
	...
	addr.sin_port = htons(WPA_SUPP_PTK_TCP_PORT);
	...
	if (listen(s, 5) < 0) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
			"PTK TCP: listen() failed");
		close(s);
		return;
	}

	if (eloop_register_read_sock(s, wpa_sm_ptk_tcp_handler, sm, NULL) < 0) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
			"PTK TCP: eloop_register_read_sock() failed");
		close(s);
		return;
	}
	...
	sm->ptk_tcp_sock = s;
	wpa_dbg(sm->ctx->msg_ctx, MSG_INFO,
		"PTK TCP: listening on 127.0.0.1:%d", WPA_SUPP_PTK_TCP_PORT);
}


static void wpa_sm_ptk_tcp_deinit(struct wpa_sm *sm)
{
	if (sm->ptk_tcp_sock >= 0) {
		eloop_unregister_read_sock(sm->ptk_tcp_sock);
		close(sm->ptk_tcp_sock);
		sm->ptk_tcp_sock = -1;
	}
}
		    ...
		    const u8 *buf, size_t len)
{
	 ...
	 * ??????????????????????????PTK????????EAPOL-Key ???
	 */
	if (sm && sm->ptk_set && sm->ptk.installed) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
			"TEST: Ignore EAPOL-Key (4-way handshake skipped)");
		return 1; /* Returns: 1 = WPA EAPOL-Key processed*/
	}
...
void wpa_sm_notify_assoc(struct wpa_sm *sm, const u8 *bssid)
{
	 ...
	 */
	extern const struct wpa_ptk g_custom_test_ptk;
	extern const u8 g_custom_test_gtk[32];
	if (sm->pairwise_cipher != WPA_CIPHER_NONE &&
	    wpa_cipher_valid_pairwise(sm->pairwise_cipher)) {
		enum wpa_alg alg = wpa_cipher_to_alg(sm->pairwise_cipher);
		int keylen = wpa_cipher_key_len(sm->pairwise_cipher);
		...
		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
			"TEST: Skip 4-way handshake, install custom PTK directly");
		...
		/* ??? PTK ?????*/
		if (keylen > 0 && (unsigned int) keylen == sm->ptk.tk_len) {
			if (wpa_sm_set_key(sm, alg, sm->bssid, 0, 1, null_rsc,
					   rsclen, sm->ptk.tk, keylen,
					   KEY_FLAG_PAIRWISE_RX_TX) == 0) {
				sm->ptk.installed = 1;
				wpa_sm_store_ptk(sm, sm->bssid, sm->pairwise_cipher,
				...
				/* ????????GTK ?????*/
				if (sm->group_cipher != WPA_CIPHER_NONE &&
				    wpa_cipher_valid_group(sm->group_cipher)) {
					enum wpa_alg gtk_alg = wpa_cipher_to_alg(sm->group_cipher);
					int gtk_len = wpa_cipher_key_len(sm->group_cipher);
					
					wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
						"TEST: Install custom GTK (len=%d)", gtk_len);
					
					if (gtk_len > 0 && gtk_len <= 32) {
						if (wpa_sm_set_key(sm, gtk_alg, broadcast_ether_addr,
								    1, 0, null_rsc, rsclen,
								    (u8 *)g_custom_test_gtk, gtk_len,
								    KEY_FLAG_GROUP_RX) == 0) {
							wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
								"TEST: Custom GTK installed");
						} else {
							wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING,
								"TEST: Failed to install custom GTK");
						}
					}
				}

				/* Force WPA state since four-way handshake is skipped */
				wpa_sm_set_state(sm, WPA_COMPLETED);
				wpa_sm_mlme_setprotection(sm, sm->bssid,
					      MLME_SETPROTECTION_PROTECT_TYPE_RX_TX,
					      MLME_SETPROTECTION_KEY_TYPE_PAIRWISE);
				eapol_sm_notify_portControl(sm->eapol, ForceAuthorized);
				eapol_sm_notify_portEnabled(sm->eapol, true);
				eapol_sm_notify_portValid(sm->eapol, true);
				if (sm->wpa_ptk_rekey) {
					eloop_cancel_timeout(wpa_sm_rekey_ptk, sm, NULL);
					eloop_register_timeout(sm->wpa_ptk_rekey, 0,
				...
				if (wpa_key_mgmt_wpa_psk(sm->key_mgmt) ||
				    ...
				    sm->key_mgmt == WPA_KEY_MGMT_OWE) {
					eapol_sm_notify_eap_success(sm->eapol, true);
					eapol_sm_notify_lower_layer_success(sm->eapol, 1);
				}
				wpa_supplicant_key_neg_complete(sm, sm->bssid, 1);

				sm->msg_3_of_4_ok = 1;
				wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
					"TEST: Custom PTK and GTK installed, 4-way handshake skipped");
				wpa_hexdump(MSG_DEBUG, "TEST: FINAL PTK.kck installed to SM", 
					(u8 *)sm->ptk.kck, sm->ptk.kck_len);
				wpa_hexdump(MSG_DEBUG, "TEST: FINAL PTK.tk installed to driver", 
					sm->ptk.tk, sm->ptk.tk_len);
				wpa_printf(MSG_DEBUG, "TEST: SM state=%d, ptk_set=%d",
					wpa_sm_get_state(sm), sm->ptk_set);
			} else {
				wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
					"TEST: Failed to install custom PTK to driver");
				wpa_hexdump(MSG_DEBUG, "TEST: Custom PTK.kck", 
					(u8 *)&g_custom_test_ptk.kck, 16);
				wpa_hexdump(MSG_DEBUG, "TEST: Custom PTK.kek", 
					(u8 *)&g_custom_test_ptk.kek, 16);
				wpa_hexdump(MSG_DEBUG, "TEST: Custom PTK.tk", 
					(u8 *)g_custom_test_ptk.tk, 16);
			}
		}
	}
}
