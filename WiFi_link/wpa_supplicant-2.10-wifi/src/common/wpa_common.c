...
const struct wpa_ptk g_custom_test_ptk = {
	.kck = {
...
/* 自定义GTK（Group Temporal Key）用于组播/广播数据加密
 * 长度取决于group cipher：
 * - CCMP: 16字节
 * - TKIP: 32字节
 * - WEP: 5/13字节
 * 这里默认使用16字节（CCMP），如果使用其他cipher需要相应调整
 * 注意：所有字节必须明确指定，否则会是未初始化的垃圾值
 */
const u8 g_custom_test_gtk[32] = {
