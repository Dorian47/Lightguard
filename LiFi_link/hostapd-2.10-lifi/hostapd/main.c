...
#define DEAUTH_PORT 7789

struct lifi_task {
    struct hostapd_data *hapd;
    u8  sta_addr[ETH_ALEN];
    char *pass;              // 如果你传的是 passphrase；若传 64-hex PMK 则存 pmk[32]
};

/*
static void lifi_setptk_cb(void *eloop_ctx, void *timeout_ctx)
{
    struct lifi_task *t = timeout_ctx;
    struct hostapd_data *hapd = t->hapd;

    // find STA
    struct sta_info *sta = ap_get_sta(hapd, t->sta_addr);
    if (!sta) {
        wpa_printf(MSG_ERROR, "[lifi] STA not found: " MACSTR, MAC2STR(t->sta_addr));
        goto out;
    }

    if (!sta->wpa_sm) {
        //have it been initialized
        sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth, sta->addr, NULL);
        if (!sta->wpa_sm) {
            wpa_printf(MSG_ERROR, "[lifi] wpa_sm init failed: " MACSTR, MAC2STR(t->sta_addr));
            goto out;
        }
    }
	...
	//change pmk
    u8 pmk[PMK_LEN]; // 32 bytes
    pbkdf2_sha1(t->pass, (const char *) ssid->ssid, ssid->ssid_len,
                4096, pmk, PMK_LEN);
    wpa_hexdump_key(MSG_DEBUG, "[lifi] PMK", pmk, PMK_LEN);

    wpa_auth_sm_event(sta->wpa_sm, WPA_ASSOC_FT);
	
    wpa_printf(MSG_INFO, "[lifi] setptk(pass→pmk) done for " MACSTR, MAC2STR(t->sta_addr));
out:
    if (t->pass) os_free(t->pass);
    os_free(t);
}
*/
...
static int setup_ptk_injection_socket(void) {
	int sockfd;
	struct sockaddr_un addr;

	sockfd = socket(AF_UNIX, SOCK_STREAM, 0);
	if(sockfd < 0) {
		wpa_printf(MSG_ERROR, "failed to create PTK injection socket: %s", strerror(errno));
		return -1;
	}

	memset(&addr, 0, sizeof(addr));
	addr.sun_family = AF_UNIX;
	strncpy(addr.sun_path, "/var/run/hostapd_ptk_inject", sizeof(addr.sun_path) -1);

	unlink(addr.sun_path);
	if(bind(sockfd, (struct sockaddr*)&addr, sizeof(addr)))
	{
		wpa_printf(MSG_ERROR, "failed to listen on PTK injection socket: %s", strerror(errno));
		close(sockfd);
		return -1;
	}
	
	wpa_printf(MSG_INFO, "PTK injection socket ready at %s", addr.sun_path);
	return sockfd;
}
