...
int wpa_auth_inject_ptk(struct wpa_state_machine *sm, enum wpa_alg alg, const u8 *ptk, size_t ptk_len);
...
int wpa_auth_derive_ptk(struct wpa_state_machine *sm, const u8 *pmk, struct wpa_ptk *ptk)
{
    if (!sm || !sm->SNonce || !sm->PMK || !sm->ANonce)
        return -1;

    return wpa_derive_ptk(sm,
                        sm->SNonce,
                        pmk,
                        PMK_LEN,
                        ptk,
...
int wpa_auth_inject_ptk(struct wpa_state_machine *sm, enum wpa_alg alg, const u8 *ptk, size_t ptk_len)
{
	...
	os_memcpy(sm->PTK.tk, ptk, ptk_len);
	sm->PTK.installed = 1;
	sm->PTK_valid = 1;
