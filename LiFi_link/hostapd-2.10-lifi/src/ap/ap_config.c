...
int csi_generate_pmk(u8 *pmk_out)
{
	...
	wpa_printf(MSG_DEBUG, "[lifi] connecting to localhost:7788");
	...
	ssize_t r = recv(sock, pmk_out, 8, 0);
	//error
	wpa_printf(MSG_DEBUG, "[lifi]pmk_from_socket: %s", pmk_out);
	/*if (r != PMK_LEN) {
		wpa_printf(MSG_DEBUG, "[lifi] error! failed to read");
        	wpa_printf(MSG_DEBUG, "[lifi] read {r} bytes");
        	close(sock);
        	return -1;
	}*/
	close(sock);
...
{	
	...
	u8 test_csi[9];
	test_csi[8]='\0';
	// with unchanged encript
	
	if(csi_generate_pmk(test_csi))
	{
		ssid->wpa_psk->group = 1;
	}
	else
	{
		wpa_printf(MSG_INFO, "[error!] failed to get psk");
	}

	wpa_hexdump_ascii(MSG_DEBUG, "SSID",
			  (u8 *) ssid->ssid, ssid->ssid_len);
	wpa_hexdump(MSG_DEBUG, "[lifi] PSK (ASCII CSI)",
			      (u8 *) test_csi,
			      os_strlen(test_csi));

	pbkdf2_sha1(test_csi,
		    ssid->ssid, ssid->ssid_len,
		    4096, ssid->wpa_psk->psk, PMK_LEN);
	
	if (!ssid->wpa_psk)
	{
		wpa_printf(MSG_ERROR, "[lifi] ssid->wpa_psk is NULL");
	}

	wpa_hexdump(MSG_DEBUG, "[lifi] PSK (from csi)",
	...
	*/
	...
	wpa_hexdump_ascii(MSG_DEBUG, "SSID",
			  (u8 *) ssid->ssid, ssid->ssid_len);
	wpa_hexdump_ascii_key(MSG_DEBUG, "[lifi] initialized PSK (ASCII passphrase)",
			      (u8 *) ssid->wpa_passphrase,
			      os_strlen(ssid->wpa_passphrase));
	pbkdf2_sha1(ssid->wpa_passphrase,
		    ssid->ssid, ssid->ssid_len,
		    4096, ssid->wpa_psk->psk, PMK_LEN);
	wpa_hexdump_key(MSG_DEBUG, "[lifi] initial PSK (from passphrase)",
			ssid->wpa_psk->psk, PMK_LEN);
	return 0;
}
...
int hostapd_setup_wpa_psk(struct hostapd_bss_config *conf)
{
	...
	if (ssid->wpa_passphrase != NULL) {
		if (ssid->wpa_psk != NULL) {
			...
			wpa_printf(MSG_DEBUG, "LIFI: Deriving WPA PSK based on "
				   "csi");
