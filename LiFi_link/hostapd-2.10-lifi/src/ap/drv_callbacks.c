...
static int hostapd_event_new_sta(struct hostapd_data *hapd, const u8 *addr)
{
	...
	wpa_printf(MSG_INFO, "[lifi] New STA: " MACSTR, MAC2STR(addr));
/*
    if (!sta->wpa_sm) {
        sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth, addr, NULL, NULL, pmk,);
        if (!sta->wpa_sm) {
            wpa_printf(MSG_ERROR, "[lifi] Failed to init WPA state machine");
            return -1;
        }
    }

    u8 pmk[PMK_LEN];
    if (!csi_generate_pmk(pmk)) {
        wpa_printf(MSG_ERROR, "[lifi] Failed to generate PMK from CSI");
        return -1;
    }

    if (wpa_auth_sta_set_pmk(sta->wpa_sm, pmk, PMK_LEN) < 0) {
        wpa_printf(MSG_ERROR, "[lifi] Failed to set PMK for STA");
        return -1;
    }

	wpa_auth_sta_associated(hapd->wpa_auth, sta->wpa_sm);

    	wpa_printf(MSG_INFO, "[lifi] PMK successfully set for STA " MACSTR,
               MAC2STR(addr));
