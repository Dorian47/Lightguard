...
#define MAC_LEN 6
//new
int fetch_csi(u8 *csi_out) {
    int sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (sockfd < 0) return -1;

    struct sockaddr_in addr = {
        .sin_family = AF_INET,
        .sin_port = htons(9911), // coordinatior
        .sin_addr.s_addr = inet_addr("127.0.0.1")
    };

    if (connect(sockfd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        wpa_printf(MSG_ERROR, "Failed to connect to CSI server: %s", strerror(errno));
        close(sockfd);
        return -1;
    }

    if (read(sockfd, csi_out, CSI_LEN) != CSI_LEN) {
        wpa_printf(MSG_ERROR, "Failed to read CSI from python server");
        close(sockfd);
        return -1;
    }
...
void send_ptk_to_wifi(const u8 *ptk) {
    int sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (sockfd < 0) return;

    struct sockaddr_in addr = {
        .sin_family = AF_INET,
        .sin_port = htons(8877), // wifi port
        .sin_addr.s_addr = inet_addr("127.0.0.1")
    };

    if (connect(sockfd, (struct sockaddr*)&addr, sizeof(addr)) == 0) {
        write(sockfd, ptk, PTK_LEN);
	wpa_hexdump_key(MSG_DEBUG, "[lifi] send PTK:", ptk, PTK_LEN);
    } else {
        wpa_printf(MSG_DEBUG, "[lifi] PTK send failed: %s", strerror(errno));
    }
...
#define MAX_Z_LEN 66 /* with NIST P-521 */
	...
	//new send to systemManager
	send_ptk_to_wifi(tmp);		
	...
	u8 csi[CSI_LEN] = {0};
   	if (fetch_csi(csi) != 0)
       	{
       		wpa_printf(MSG_ERROR, "Failed to fetch CSI, abort PTK derivation");
        	return -1;
   	} 
	u8 buffer[256];
	os_memcpy(buffer, pmk, pmk_len);
	os_memcpy(buffer + pmk_len, csi, CSI_LEN);
	
	if(os_memcmp(nonce1, nonce2, WPA_NONCE_LEN) < 0) {
		os_memcpy(buffer + pmk_len + CSI_LEN, nonce1, WPA_NONCE_LEN);
		os_memcpy(buffer + pmk_len + CSI_LEN + WPA_NONCE_LEN, nonce2,WPA_NONCE_LEN);	
	}
	else
	{
		os_memcpy(buffer + pmk_len + CSI_LEN, nonce2, WPA_NONCE_LEN);
        	os_memcpy(buffer + pmk_len + CSI_LEN + WPA_NONCE_LEN, nonce1, WPA_NONCE_LEN);
	}
	ptk->kck_len = wpa_kck_len(akmp, pmk_len);
        ptk->kek_len = wpa_kek_len(akmp, pmk_len);
        ptk->kdk_len = kdk_len;
	//ptk_tk_len is PTK_LEN
	
        os_memcpy(ptk->kck, buffer, ptk->kck_len);
        wpa_hexdump_key(MSG_DEBUG, "WPA: KCK", ptk->kck, ptk->kck_len);

        os_memcpy(ptk->kek, buffer + ptk->kck_len, ptk->kek_len);
        wpa_hexdump_key(MSG_DEBUG, "WPA: KEK", ptk->kek, ptk->kek_len);

	
	u8 derived_ptk[PTK_LEN];
	//simply use 256
    	sha256_prf(pmk, pmk_len, label, buffer, pmk_len + CSI_LEN + WPA_NONCE_LEN*2, derived_ptk, PTK_LEN);
	
	os_memcpy(ptk->tk, derived_ptk, PTK_LEN);
	wpa_hexdump_key(MSG_DEBUG, "WPA: TK", ptk->tk, PTK_LEN);

	//kdk is not used in personal
	
	
	// send to wifi 
	if (os_memcmp(addr1, addr2, ETH_ALEN) < 0) {
	      	send_ptk_to_wifi(addr2, derived_ptk);
	}
	else {
		send_ptk_to_wifi(addr1, derived_ptk);
	}	
	return 0;
	*/
}
