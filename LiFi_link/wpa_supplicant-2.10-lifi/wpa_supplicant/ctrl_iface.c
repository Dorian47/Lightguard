...
//new
static int wpa_s_ctrl_iface_set_lifi_pass(struct wpa_supplicant *wpa_s, char *cmd)
{
    wpa_printf(MSG_INFO, "[lifi] change wpa_s...");
    if (!wpa_s->wpa) return -1;
    return  wpa_sm_set_passphrase(wpa_s->wpa, cmd);
}
					 ...
					 char *buf, size_t *resp_len)
{
	...
	int reply_len;
	//new
	wpa_printf(MSG_INFO, "CTRL_IFACE recieved: %s\n", buf);
	if (os_strncmp(buf, "SET_LIFI_PASS ", 14) == 0) {
        	char *arg = buf + 14;
		wpa_printf(MSG_DEBUG, "[lifi] checkpoint1!");

        	int ret = wpa_s_ctrl_iface_set_lifi_pass(wpa_s, arg);
	    	
		wpa_printf(MSG_DEBUG, "[lifi] checkpoint2!");
		/*
		if (ret ==0)
	 	 {
			 *resp_len = os_snprintf(reply, reply_size, "OK\n");
		 }
		*resp_len = os_snprintf(reply, reply_size, "OK\n");
		*/
    	}
