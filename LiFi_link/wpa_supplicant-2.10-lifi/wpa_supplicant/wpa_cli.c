...
//new
static int wpa_cli_cmd_setpsk(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	if (argc != 1) {
	...
	printf("setting psk...");
	char cmd[256];
	snprintf(cmd, sizeof(cmd), "SET_LIFI_PASS %s", argv[0]);
	printf("cmd: %s", cmd);
	return wpa_ctrl_command(ctrl, cmd);
}
...
static const struct wpa_cli_cmd wpa_cli_commands[] = {
	{ "setpsk", wpa_cli_cmd_setpsk, NULL, 
		cli_cmd_flag_none, 
	      	"<ptk> = manually inject new password for specific STA"	},
