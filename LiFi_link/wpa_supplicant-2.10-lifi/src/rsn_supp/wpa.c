...
#define PMK_LEN 32
int wpa_sm_set_passphrase(struct wpa_sm *sm, const char *pass)
{
    size_t len;
    if (!sm || !pass) return -1;
    len = os_strlen(pass);
    if (len >= sizeof(sm->lifi_passphrase)) return -1;
    os_memcpy(sm->lifi_passphrase, pass, len);
    sm->lifi_passphrase[len] = '\0';
    sm->lifi_passphrase_valid = 1;
    wpa_printf(MSG_INFO, "[lifi] wpa_sm: LiFi new passphrase saved");
    return 0;
}

/*
static void wpa_try_update_pmk_from_lifi(struct wpa_sm *sm)
{
    if (sm->lifi_passphrase_valid && sm->ssid) {
        u8 pmk[PMK_LEN];
        pbkdf2_sha1(sm->lifi_passphrase,
                    sm->ssid, sm->ssid_len,
                    4096, pmk, PMK_LEN);
        os_memcpy(sm->pmk, pmk, PMK_LEN);
        sm->pmk_len = PMK_LEN;
        wpa_hexdump_key(MSG_DEBUG, "wpa_sm: PMK re-derived from LiFi passphrase",
                        sm->pmk, sm->pmk_len);
    }
}
*/
					  ...
					  size_t key_data_len)
{
	...
	if(sm->lifi_passphrase_valid && sm->ssid && sm->ssid_len > 0) {
        u8 pmk[PMK_LEN];
        pbkdf2_sha1(sm->lifi_passphrase,
                    sm->ssid, sm->ssid_len,
                    4096, pmk, PMK_LEN);
        os_memcpy(sm->pmk, pmk, PMK_LEN);
        sm->pmk_len = PMK_LEN;
        /* Clear PMKSA cache entry to prevent wpa_supplicant_get_pmk() 
         * from overwriting our new PMK with the cached old one */
        sm->cur_pmksa = NULL;
        wpa_printf(MSG_INFO, "[lifi] PMK re-derived from passphrase, PMKSA cache cleared");
        wpa_hexdump_key(MSG_DEBUG,
                        "wpa_sm: PMK re-derived from LiFi passphrase",
                        sm->pmk, sm->pmk_len);
        /* Clear the passphrase flag so we don't re-derive on next M1 retry */
        sm->lifi_passphrase_valid = 0;
    	}
		    ...
		    const u8 *pmkid, const u8 *bssid)
{
	...
	//new
	u8 csi_pmk[32];
	csi_generate_pmk(csi_pmk);
	
	wpa_hexdump_key(MSG_DEBUG, "WPA: Set PMK based on external csi",
			csi_pmk, PMK_LEN);
	sm->pmk_len = PMK_LEN;
	os_memcpy(sm->pmk, csi_pmk, PMK_LEN);
	*/
...
//	sm->xxkey_len = PMK_LEN;
//	os_memcpy(sm->xxkey, csi_pmk, PMK_LEN);
